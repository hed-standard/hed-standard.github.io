...
(function() {
  ...
  const CONFIG = {
    ...
    // Tooltip text
    // tooltipText: 'Chat with HED assistant',
    // tooltipCloseText: 'Close chat',
  ...
  function createWidget() {
    ...
    container.innerHTML = `
      ...
      <div class="osa-chat-tooltip">${escapeHtml(CONFIG.tooltipText)}</div>
  ...
  function toggleChat(container) {
    ...
    if (isOpen) {
      ...
      if (tooltip) tooltip.textContent = CONFIG.tooltipCloseText;
      container.querySelector('.osa-chat-input input').focus();
    } else {
      chatWindow.classList.remove('open');
      button.innerHTML = ICONS.chat;
      button.setAttribute('aria-label', 'Open chat');
      if (tooltip) tooltip.textContent = CONFIG.tooltipText;
    }
  }
